...
function initBoard(type, boardElId, countElId) {
  ...
  // Set today's date as default in upload modal
...
function buildDocItem(doc) {
  ...
  const categoryBadge = doc.category
    ? `<span style="background: rgba(79,126,247,0.1); color: var(--blue-light); padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600;">${doc.category}</span>`
    : '';

  return `
    <div class="board-item" onclick="viewDocument('${doc.id}')">
...
function processFile(file, type) {
  ...
  if (titleInput && !titleInput.value) {
    const name = file.name.replace('.pdf', '').replace(/_/g, ' ').replace(/-/g, ' ');
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    ...
    if (viewer && viewer.classList.contains('open')) closeViewerModal();
    else if (upload && upload.classList.contains('open')) closeUploadModal();
  }
});
